-- HBG Vip Hub - Fixed (Giữ nguyên tính năng)
-- by Huy Báo Game
...
-- ============================================
-- FIX: KIỂM TRA TỒN TẠI TRƯỚC KHI HOOK
-- ============================================
local function safeHook(modulePath, callback)
    local success, module = pcall(function()
        return require(modulePath)
    end)
    if success and module then
        pcall(function()
            hookfunction(module, callback)
        end)
    end
end

-- Hook Death/Respawn an toàn
local deathPath = game:GetService("ReplicatedStorage"):FindFirstChild("Effect")
if deathPath then
    deathPath = deathPath:FindFirstChild("Container")
    if deathPath then
        safeHook(deathPath.Death, function() end)
        safeHook(deathPath.Respawn, function() end)
    end
end
...
-- ============================================
-- ĐỔI SUBTITLE THÀNH "HBG Hub Vip"
-- ============================================
-- Lưu ý: Phần này cần được thực hiện SAU KHI UI được tạo
-- Script gốc sẽ tạo UI ở dòng 3739, vì vậy chúng ta cần hook vào đó

-- ============================================
-- TIẾP TỤC CÁC HÀM KHÁC (GIỮ NGUYÊN)
-- ============================================
function Hop()
    -- ... (giữ nguyên)
end
